...
| **Geospatial / Routing** | Compute optimal drone flight paths, own geofence boundary definitions, enforce no-fly zones, integrate with maps API, reroute around dynamic obstacles; evaluates drone position against geofences on each telemetry ping | H3 / S2, Maps API, Polly | Go | None | Consumes `telemetry.ping`; publishes `geofence.entered`, `geofence.exited`; gRPC (flight path + routing queries) | ✅ Yes | ❌ Missing |
...
| **Scheduler / Optimizer** | Determine dispatch timing, batch nearby orders to the same drone, respect delivery windows and charge schedules | Constraint solver, MassTransit | .NET | None | gRPC (receives dispatch requests from Temporal Activities); publishes `dispatch.scheduled` | ✅ Yes | ❌ Missing |
...
2. The workflow executes a sequence of **Activities**, each of which calls a downstream service via gRPC:
   ...
   - `RequestDispatchActivity` → Scheduler / Optimizer gRPC; hands off the order ID, assigned drone ID, and delivery window; the Scheduler performs batching and timing optimisation and then publishes `dispatch.scheduled` to the bus when ready
   - `AwaitDispatchSignal` → workflow pauses; resumes when the `dispatch.scheduled` signal arrives (see below)
...
3. On any Activity failure, Temporal drives compensation in reverse order:
   - `CancelDispatchActivity` → Scheduler / Optimizer gRPC (if `RequestDispatchActivity` had already succeeded)
...
Fulfillment Orchestrator (MassTransit consumer)
              │
              └──SignalAsync("DispatchReady")──▶ FulfillmentWorkflow resumes
```
...
> **Note:** The Fulfillment domain has no bus topics — all saga coordination is internal to the Temporal workflow. The Scheduler / Optimizer is its own bounded context; `dispatch.scheduled` belongs to the Scheduling domain, not Fulfillment, even though the Fulfillment Orchestrator consumes it.
...
|---|---|---|---|---|
| `drone.assigned` | Fleet Management | Shipping & Tracking, Audit Log | | |
...
| `telemetry.ping` | Drone Telemetry Hub | Shipping & Tracking, Geospatial / Routing, Regulatory | | High frequency — short retention, consumers must tolerate gaps |
...
| `geofence.entered` | Geospatial / Routing | Shipping & Tracking, Notification Engine, Regulatory | |
| `geofence.exited` | Geospatial / Routing | Shipping & Tracking, Regulatory, Audit Log | |
...
**Scheduler handoff is explicit.** The workflow calls the Scheduler / Optimizer via `RequestDispatchActivity` (gRPC) rather than relying on the Scheduler to independently notice a `drone.assigned` event. This makes the trigger point observable — if the Scheduler is unavailable, the Activity fails fast with a clear error in the Temporal workflow history rather than the workflow silently stalling at `AwaitDispatchSignal`.
...
**Geospatial / Routing owns all spatial evaluation.** Geofence boundary definitions live in the Geospatial / Routing service alongside no-fly zones and flight path data — all spatial concerns are collocated. Geospatial consumes `telemetry.ping` directly and publishes `geofence.entered` / `geofence.exited` when a boundary crossing is detected. Shipping & Tracking consumes those events to update delivery state and has no knowledge of geofence geometry, keeping its responsibility narrow: delivery progress and ETAs only.
